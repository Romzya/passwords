...
def register():
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('register.html', error='нет')
...
def login():
    if request.method == 'POST':
        ...
        if user:
            ...
            return render_template('login.html', error='нет')
...
def add_password():
    if 'user_id' not in session:
        ...
        if request.method == 'POST':
            ...
            conn.commit()
            conn.close()
        return render_template('index.html', error='нет')
    return redirect('/index')
...
def delete_password(password_id):
    if 'user_id' not in session:
        ...
        conn = sqlite3.connect('my_database.db')
        c = conn.cursor()
        c.execute("DELETE FROM BOB WHERE id = ?", (password_id))
        conn.commit()
        conn.close()
